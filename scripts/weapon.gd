...
## Weighted random pick from a Dictionary {key: weight}. Returns the key.
static func weighted_pick(weights: Dictionary):
        var total := 0
...
static func roll_affliction_from_death(enemy_type: String, current_wear: int) -> Dictionary:
        ...
        var new_wear: int = weighted_pick(filtered)
...
static func roll_affliction(p_type: String) -> Dictionary:
        ...
        var wear: int = weighted_pick(weights)
        ...
        match wear:
                ...
                _:                  dur_pct = 0.35
        # --- Flavor state from wear + haunt (single source of truth) ---
        var flavor: int = _derive_flavor(wear, haunted, p_type)
        return {
                "state": flavor,
                "wear_state": wear,
...
# Wear state → stat multiplier mapping (single source of truth)
const WEAR_MULTS := {
        WearState.PRISTINE: 1.0,
        WearState.WORN: 0.85,
        WearState.DAMAGED: 0.6,
        WearState.BROKEN: 0.0,
}
const DREAD_PER_DEATH := 0.06
...
func stat_multiplier() -> float:
        ...
        var wear_mult: float = WEAR_MULTS[wear_state]
        ...
        var dread_penalty := clampf(float(unexorcised_deaths) * DREAD_PER_DEATH, 0.0, DREAD_MAX)
...
## Derive flavor state from (wear, haunted, type). Single source of truth —
## used by both recalculate_state() and roll_affliction() to prevent drift.
static func _derive_flavor(wear: int, haunted: bool, p_type: String) -> int:
        if wear == WearState.BROKEN:
                return State.SHATTERED
        if haunted:
                return State.HAUNTED
        if wear == WearState.PRISTINE:
                return State.PRISTINE
        if p_type in ["sword", "staff"]:
                return State.BLOODIED
        return State.RUSTED

## Derive flavor state from current wear/haunt/type. Call after ANY durability
## or haunt change or the flavor state drifts from the mechanical truth.
func recalculate_state() -> void:
        state = _derive_flavor(wear_state, is_haunted(), type)
