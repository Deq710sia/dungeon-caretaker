...
func _draw() -> void:
	...
	for y in range(cam_top, cam_bot + 1):
		...
		if y % 4 == 0:
			...
			# Ambient torch glow (radial gradient)
	...
	for prop in props:
		if prop.pos.y > cam_top * TILE - 16 and prop.pos.y < cam_bot * TILE + 16:
			draw_texture(Sprites.get_sprite(prop.sprite), prop.pos)
	# Hazards
	for h in hazards:
		if h.active:
			match h.type:
				"pit":
					draw_texture(Sprites.get_sprite("pit"), h.pos - Vector2(8, 8))
				"fire":
				...
				"spikes":
					...
					for i in 3:
						...
						draw_rect(Rect2(sx, sy - 4, 1, 8), Palette.STEEL, true)
	...
	for c in corpses:
		if c.collected:
			...
			# Glow under floating gear
			_draw_gear_glow(c.pos + Vector2(0, -12 + bob))
	...
	var gp: Vector2 = ghost_pos + Vector2(0, bob)
