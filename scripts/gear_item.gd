...
# V2: Durability system — weapons degrade during battle
...
func take_durability_damage(amount: int) -> void:
        durability = max(0, durability - amount)
        if durability <= 0:
                # Weapon broke — degrade state
                match state:
                        State.PRISTINE: state = State.BLOODIED
                        State.BLOODIED: state = State.RUSTED
                        State.RUSTED:   state = State.SHATTERED
                        State.HAUNTED:  state = State.CURSED
                        State.CURSED:   state = State.SHATTERED
                        State.SHATTERED: pass
                durability = durability_max  # reset for next state
                history.append("Broke during battle on Stage %d Wave %d." % [GameState.stage, GameState.wave])
