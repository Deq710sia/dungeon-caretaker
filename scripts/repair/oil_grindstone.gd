...
func _draw() -> void:
	...
	# Per-item-type contact positions: each weapon type grinds at a
	# different point — swords grind at the blade (not the hilt), staves
	# grind at the tip, helms grind at the rim, robes grind at the hem.)
	if gear != null:
		var weapon_tex := Sprites.get_weapon_sprite_wear(gear.type, gear.wear_state, gear.is_haunted())
		var tex_size := 64
		# Per-type grind position: the contact angle on the wheel and the
		# weapon's offset/tilt. The 16x16 sprite is scaled 4x to 64px, so
		# the "contact edge" of each weapon type is at a different pixel
		# offset within that 64px quad:
		#   sword — blade is at the top half, so offset weapon so blade
		#           touches the wheel (weapon_center is below contact)
		#   staff — tip is at top, similar to sword but different tilt
		#   helm  — rim is at bottom, so weapon is above the contact
		#   robe  — hem is at bottom, weapon above contact, more tilt
		var contact_angle: float
		var weapon_tilt: float
		var contact_offset: Vector2  # offset from contact_point to weapon center
		match gear.type:
			"sword":
				# Blade grinds — sword is held above the wheel, blade tip
				# pointing down-left into the contact point.
				contact_angle = -PI * 0.75  # upper-left of wheel
				weapon_tilt = -0.5  # tilted so blade points toward wheel
				# Weapon center is ABOVE-RIGHT of contact (blade tip at contact)
				contact_offset = Vector2(tex_size * 0.3, -tex_size * 0.25)
			"staff":
				# Tip grinds — staff is held at an angle, tip at the wheel
				contact_angle = -PI * 0.70
				weapon_tilt = -0.7  # steeper angle for a staff
				contact_offset = Vector2(tex_size * 0.25, -tex_size * 0.3)
			"helm":
				# Rim grinds — helm sits on top of the wheel, rim at contact
				contact_angle = -PI * 0.5  # top of wheel (12 o'clock)
				weapon_tilt = 0.0  # flat, rim sits on wheel
				contact_offset = Vector2(0, -tex_size * 0.35)
			"robe":
				# Hem grinds — robe draped over the wheel, hem at contact
				contact_angle = -PI * 0.5
				weapon_tilt = 0.2  # slight tilt
				contact_offset = Vector2(tex_size * 0.1, -tex_size * 0.3)
			_:
				contact_angle = -PI * 0.75
				weapon_tilt = -0.4
				contact_offset = Vector2(tex_size * 0.3, -tex_size * 0.25)
		var contact_point := wheel_center + Vector2(cos(contact_angle), sin(contact_angle)) * wheel_r
		var weapon_center := contact_point + contact_offset
		...
		# Sparks fly off the CONTACT POINT when oil flows
		if oil_level > 0.3:
			for i in 5:
		...
		# Draw weapon with type-specific tilt
	...
	# Label
	GameFont.draw_string_centered(self, Vector2(meter_x + 10, meter_y - 4), "OIL", 8, Color(0.85, 0.85, 0.90))
	# Stats
