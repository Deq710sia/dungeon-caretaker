...
# Hammer grid layout — shared between _draw_hammer and _click_hammer (drift bug fix)
const HAMMER_COLS: int = 8
const HAMMER_ROWS: int = 4
const HAMMER_CELL_W: float = 32.0
const HAMMER_CELL_H: float = 12.0
const HAMMER_GX: float = int(480 / 2 - HAMMER_COLS * HAMMER_CELL_W / 2)
const HAMMER_GY: float = 104.0

# Melt crucible layout — shared between _draw_melt and _click_melt (drift bug fix)
const MELT_CRUC_Y: float = 100.0
const MELT_CRUC_H: float = 36.0
const MELT_LOCK_W: int = 60
const MELT_LOCK_H: int = 16
const MELT_LOCK_OFFSET: int = 6
...
func _draw_melt() -> void:
        ...
        var cruc_y: float = MELT_CRUC_Y
        var cruc_w: float = vp.x * 0.50
        var cruc_h: float = MELT_CRUC_H
        # Background
        ...
        var lock_rect := Rect2(int(vp.x / 2 - MELT_LOCK_W / 2), int(MELT_CRUC_Y + MELT_CRUC_H + MELT_LOCK_OFFSET), MELT_LOCK_W, MELT_LOCK_H)
...
func _draw_hammer() -> void:
        var vp := Vector2(480, 270)
        var cols: int = HAMMER_COLS
        var rows: int = HAMMER_ROWS
        var cell_w: float = HAMMER_CELL_W
        var cell_h: float = HAMMER_CELL_H
        var gx: float = HAMMER_GX
        var gy: float = HAMMER_GY
        for y in rows:
                for x in cols:
                        var idx := y * cols + x
...
func _click_melt(pos: Vector2) -> void:
        var vp := Vector2(480, 270)
        # Match the lock_rect from _draw_melt (uses same consts — no drift)
        var lock_rect := Rect2(int(vp.x / 2 - MELT_LOCK_W / 2), int(MELT_CRUC_Y + MELT_CRUC_H + MELT_LOCK_OFFSET), MELT_LOCK_W, MELT_LOCK_H)
