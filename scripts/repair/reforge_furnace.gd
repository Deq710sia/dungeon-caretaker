...
func _draw() -> void:
	...
	if stage < 3:
		draw_string(GameFont.get_font(), Vector2(0, 44), stage_names[stage], HORIZONTAL_ALIGNMENT_CENTER, -1, 9, Color(0.95, 0.85, 0.40))
	...
	if gear != null:
		...
		# Name
		draw_string(GameFont.get_font(), Vector2(vp.x / 2 - 60, wpos.y + wsize + 14), gear.display_name, HORIZONTAL_ALIGNMENT_CENTER, -1, 7, gear.wear_color())
	match stage:
		0: _draw_melt()
		1: _draw_pour()
		2: _draw_hammer()
		3:
			draw_string(GameFont.get_font(), Vector2(vp.x / 2 - 60, vp.y / 2), "COMPLETE!", HORIZONTAL_ALIGNMENT_CENTER, -1, 16, Color(0.55, 0.95, 0.75))
...
func _draw_melt() -> void:
	...
	draw_string(GameFont.get_font(), lock_rect.position + Vector2(15, 13), "LOCK", HORIZONTAL_ALIGNMENT_CENTER, -1, 8, Color(1, 1, 1))
	if melt_locked:
		draw_string(GameFont.get_font(), Vector2(vp.x / 2 - 40, lock_rect.position.y + 30), "Score: %.0f%%" % (melt_score * 100), HORIZONTAL_ALIGNMENT_CENTER, -1, 8, Color(0.55, 0.95, 0.75))
	else:
		draw_string(GameFont.get_font(), Vector2(vp.x / 2 - 80, lock_rect.position.y + 30), "Hold mouse to melt. Lock in green zone.", HORIZONTAL_ALIGNMENT_CENTER, -1, 7, Color(0.85, 0.85, 0.85))
...
func _draw_pour() -> void:
	...
	if not pour_locked:
		draw_string(GameFont.get_font(), Vector2(vp.x / 2 - 80, bar_y + bar_h + 14), "Click when marker hits green!", HORIZONTAL_ALIGNMENT_CENTER, -1, 7, Color(0.85, 0.85, 0.85))
	else:
		draw_string(GameFont.get_font(), Vector2(vp.x / 2 - 40, bar_y + bar_h + 14), "Score: %.0f%%" % (pour_score * 100), HORIZONTAL_ALIGNMENT_CENTER, -1, 8, Color(0.55, 0.95, 0.75))
...
func _draw_hammer() -> void:
	...
	draw_string(GameFont.get_font(), Vector2(vp.x / 2 - 60, gy + rows * cell_h + 14), "Click cells! Swings: %d" % max(0, hammer_swings), HORIZONTAL_ALIGNMENT_CENTER, -1, 8, Color(0.85, 0.85, 0.85))
