...
const DRIFT_DIVE_TO_FLOAT: float = 0.25     # 8-frame stop (dive ends → FLOAT, ride the burst)
...
func _update_dive(input_dir: Vector2, delta: float) -> void:
	...
	if input_dir != Vector2.ZERO:
		...
		pass
	pos += vel * delta
	# When dive mult decays enough, transition to FLOAT + refill pulse charges
...
## Dive ends — transition to FLOAT + refill pulse charges.
## This is the chain link: dive completion = 2 new pulse charges.
func _end_dive() -> void:
	state = State.FLOAT
	_dive_mult = 1.0
	# Refill pulse charges — the chain reward
